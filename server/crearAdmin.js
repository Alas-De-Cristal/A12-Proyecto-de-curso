  ...
  const manejarEnvio = async (e) => {
    ...
    if (esRegistro) {
      ...
      if (!regex.test(datos.password)) {
        setCargando(false);
        return alert(
          "❌ La contraseña debe ser robusta (8+ caracteres, Mayúscula, Número y Especial)."
        );
      }

      try {
        const rolFinal =
          datos.correo === "mtayan1353@utm.edu.ec" ? "Admin" : "Voluntario";
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {esRegistro
            ? "Completa tu perfil de colaborador"
        ...
        >
          {esRegistro && (
            <>
              ...
              <input
                ...
                onChange={(e) =>
                  setDatos({ ...datos, organizacion: e.target.value })
                }
              />
              <input
                type="text"
          ...
          <button type="submit" style={estiloInput} disabled={cargando}>
            {cargando
              ? "PROCESANDO..."
              : esRegistro
              ? "FINALIZAR REGISTRO"
